...
public class Lane extends Thread implements PinsetterObserver, Serializable {
	...
	private LaneScore scoreHandler;
	...
	public void partyAssignedAndGameNotFinished() {
		...
		if (bowlerIterator.hasNext()) {
			...
			ball = 0;
			while (canThrowAgain) {
				setter.ballThrown(currentThrower.getNick());		// simulate the thrower's ball hiting
				ball++;
			}
	...
	public void assignParty( Party theParty ) {
		party = theParty;
		scoreHandler = new LaneScore(this, party);
		resetBowlerIterator();
		partyAssigned = true;
		int [][] cumulScores = scoreHandler.getCumulScores();
